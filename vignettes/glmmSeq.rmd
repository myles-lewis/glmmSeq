...
[![Downloads](https://cranlogs.r-pkg.org/badges/grand-total/glmmSeq?color=orange)](https://cran.rstudio.com/package=glmmSeq)
...
from `MASS` and the `glmer` function in `lme4` which enables random 
...
and six-month EULAR response. Where EULAR is a rheumatoid arthritis response metric. 
- tpm: the transcript per million RNA-seq count data
- dispersion: The dispersion for each gene expression
...
Alternatively DESeq2 (estimateDispersions) and edgeR (estimateCommonDisp) provide 
their own dispersion calculations which can also be passed in as a named vector. 
...
this analysis can take some time, width 2 cores:
...
a significance table based on the cut-off (default p=0.05):
...
Plots can be viewed using either ggplot:
...
```{r}
labels = c("IGHV7-4-1", "IGHV3-49", "IGHV3-23", "ADAM12", "FGFRL1", "IL36G",
           "BLK", "SAA1", "CILP", "EMILIN3", "EMILIN2", "IGHJ6", "CXCL9")
...
timepoints which to not present at others. This time we can use ggplot graphics:
