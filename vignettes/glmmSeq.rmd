...
```{r}
stats = data.frame(results@stats$pvals)

kable(stats[order(stats[, 'Timepoint:EULAR_6m']), ]) %>%
  kable_styling() %>%
...
add a matrix called `qvals` to the `@stats` slot:
...
The comparative fold change (for x1var variables) between conditions 
(x2var and x2Values variables) can be plotted using fcPlots 
